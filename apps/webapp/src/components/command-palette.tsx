          ...
          <Command.List className="max-h-72 overflow-y-auto p-2">
            ...
            >
              <Command.Item
                ...
                onSelect={() =>
                  runAction(() =>
                    window.dispatchEvent(new Event("datapeek:execute")),
                  )
                }
              >
                <Play className="size-4" />
                Run Query
                <kbd className="ml-auto text-[10px] opacity-50 font-mono">
                  ⌘↵
                </kbd>
              </Command.Item>
              <Command.Item
                ...
                onSelect={() =>
                  runAction(() =>
                    window.dispatchEvent(new Event("datapeek:format")),
                  )
                }
              >
                ...
                >
                  {connections.map((conn) => (
                    <Command.Item
                      ...
                      className="flex items-center gap-2 rounded-md px-2 py-2 text-sm cursor-pointer aria-selected:bg-accent/10 aria-selected:text-foreground text-muted-foreground"
                      onSelect={() =>
                        runAction(() => setActiveConnection(conn.id))
                      }
                    >
                      <div
