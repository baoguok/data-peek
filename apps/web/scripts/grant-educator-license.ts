...
if (!["pro", "team", "enterprise"].includes(plan)) {
  console.error(
    `Error: Invalid plan "${plan}". Must be one of: pro, team, enterprise`,
  );
  process.exit(1);
}
...
`);
  ...
  if (isDryRun) {
    ...
    console.log(
      `  3. Insert license into database (marked as educator from ${institution})`,
    );
  ...
  try {
    ...
    if (existingLicense.length > 0 && existingLicense[0].status === "active") {
      ...
      console.log(
        `  Expires: ${existingLicense[0].updatesUntil.toLocaleDateString()}`,
      );
    ...
    const maxActivations =
      plan === "team" ? 10 : plan === "enterprise" ? 100 : 3;
