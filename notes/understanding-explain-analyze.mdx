...
**EXPLAIN** asks the query planner: "Hey, what _would_ you do if I ran this query?" It returns a plan based on table statistics, without actually touching any data. It's fast, safe, and gives you estimates.

```sql
EXPLAIN SELECT * FROM orders WHERE customer_id = 42;
```

**EXPLAIN ANALYZE** says: "Actually run the query and tell me what _really_ happened." It executes the query, measures real timings, and compares the planner's estimates to reality.
...
Each individual execution is fast (0.018ms), but multiply by 85,000 loops and you've spent 1.5 seconds on this one node. The `actual time` values are _per loop_. Total time is `actual time * loops`.
