...
But we didn't stop there. We built a system that understands _intent_:
...
_This is how we built the AI assistant in data-peek. The code is open source - check out the implementation in `src/main/ai-service.ts` and `src/renderer/src/components/ai/`._
