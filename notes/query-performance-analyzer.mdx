...
You write a SQL query. It works. But is it _fast_? Most developers don't know until production slows to a crawl.
...
function analyzePlanTree(node: ExplainNode, issues: PerformanceIssue[]): void {
...
ipcMain.handle(
  ...
  async (_, { config, query, queryHistory, analysisConfig }) => {
    // PostgreSQL only for now
    if (config.dbType && config.dbType !== "postgresql") {
      return {
        success: false,
        error:
          "Performance analysis is currently only supported for PostgreSQL",
      };
    }
...
export function useTabPerfIndicator(tabId: string) {
  ...
  return {
    ...
    setResult: (r: PerformanceAnalysisResult | null) =>
      store.setResult(tabId, r),
...
function PerfIssueCard({ issue }: { issue: PerformanceIssue }) {
  ...
  return (
    <div
      className={cn("rounded-lg border p-3", severityColors[issue.severity])}
    >
...
const handleAnalyzePerformance = useCallback(async () => {
  ...
  try {
    const historyForAnalysis = queryHistory.slice(0, 100).map((h) => ({
      query: h.query,
      timestamp:
        h.timestamp instanceof Date ? h.timestamp.getTime() : h.timestamp,
      duration: h.duration,
    }));
...
_This is how we built the performance analyzer in data-peek. The implementation lives in `src/main/performance-analyzer.ts` and `src/renderer/src/components/perf-indicator-panel.tsx`._
